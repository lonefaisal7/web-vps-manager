...
echo -e "${YELLOW}[2/8] Installing vnStat (persistent network monitor)...${NC}"
...
vnstat -u -i "$PRIMARY_IFACE" 2>/dev/null || true
...
echo -e "${YELLOW}[5/8] Installing Python dependencies into venv...${NC}"
"$VENV_DIR/bin/pip" install --quiet --upgrade pip
"$VENV_DIR/bin/pip" install --quiet -r "$INSTALL_DIR/requirements.txt"
echo "Dependencies installed."

echo -e "${YELLOW}[6/8] Downloading xterm.js assets...${NC}"
XTERM_VER="5.3.0"
...
echo -e "${YELLOW}[8/8] Making scripts executable...${NC}"
...
echo -e "${GREEN}║  📊 vnStat: network stats persistent    ║${NC}"
